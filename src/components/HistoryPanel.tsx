...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({ voteHistory, proposalHistory }) => {
  const renderRecord = (record: VoteRecord, type: 'candidate' | 'proposal') => (
    <div key={`${type}-${record.blockNumber}-${record.voter}`} className="grid grid-cols-4 gap-2 text-sm py-2 border-b last:border-b-0">
  ...
  return (
    <div className="mt-10 grid gap-6 md:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">Candidate Vote History</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">Proposal Vote History</CardTitle>
        </CardHeader>
        <CardContent>
